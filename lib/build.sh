...
using clang-darwin : : "/usr/local/bin/clang++" : <cxxflags>-std=c++17 ;
EOF
...
make CXX=clang++ LD=clang++ CC=clang OPTJ=-j20                                 \
     ...
     BOOST_DEFINES="-sHAVE_ICONV=1 -sHAVE_ICU=1 -sICU_PATH=/usr/local/opt/icu4c cxxflags=\"-g -std=c++17 $* -nostdlibinc -isystem /usr/local/include -isystem /usr/local/include/c++/v1 -isystem /usr/include -stdlib=libc++\" linkflags=\"-g $* -L/usr/local/lib -L/usr/lib /usr/local/lib/libc++.dylib -stdlib=libc++\""
